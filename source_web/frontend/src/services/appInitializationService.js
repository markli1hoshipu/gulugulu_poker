...
class AppInitializationService {
  ...
  async _preloadSemanticModel() {
    try {
      console.log('🧠 Preloading semantic matching model...');
      await semanticMatchingService.preloadModel();
      console.log('✅ Semantic model preload completed');
    } catch (error) {
      console.warn('⚠️ Semantic model preload failed:', error);
      // Don't fail initialization if model preload fails
    }
  }
