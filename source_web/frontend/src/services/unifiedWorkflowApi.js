...
class UnifiedWorkflowApiService {
  ...
  async getHeaders() {
    ...
    // If no proper auth token, try to get a dev token for testing
    if (!token || token === 'null' || token === 'undefined') {
      try {
        const devTokenResponse = await fetch(`${API_BASE_URL}/dev-token`);
        if (devTokenResponse.ok) {
          const devTokenData = await devTokenResponse.json();
          token = devTokenData.token;
          // Optionally store the dev token for subsequent calls
          localStorage.setItem('dev_token', token);
        }
      } catch (error) {
        console.warn('Could not get dev token:', error);
      }
    }
