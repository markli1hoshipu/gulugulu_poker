...
class PerplexityApiService {
  ...
  async getDevToken() {
    try {
      console.log('🔑 Getting dev token from:', `${API_BASE_URL}/dev-token`);
      ...
      console.log(`📡 Dev token response: ${response.status}`);
      
      if (!response.ok) {
        const errorText = await response.text();
        console.error(`❌ Failed to get dev token: ${response.status} - ${errorText}`);
        throw new Error(`Failed to get dev token: ${response.status} - ${errorText}`);
      }
      
      const data = await response.json();
      console.log('📄 Dev token data:', data);
