...
const ActivityPanel = ({
  ...
  const getEventStatus = (event, eventDate) => {
    // Don't show status for meeting events or call events
    if (event.originalType === 'meeting' || event.originalType === 'call') {
      return null;
    }
                          ...
                          <div className="flex gap-4" onClick={() => handleEventClick(event)}>
                            ...
                            <div className="flex-1 min-w-0">
                              <div className="flex items-start justify-between gap-2 mb-1">
                                ...
                                {!isNote && status && (
                                  <span className={`px-2 py-0.5 text-xs font-medium rounded-full flex-shrink-0 ${status.color}`}>
                                    {status.text}
                                  </span>
                                )}
                              </div>
