...
const UsageAnalytics = () => {
  ...
  const loadDetailedAnalytics = async (days) => {
    try {
      // const response = await fetch(`http://localhost:8005/api/analytics/detailed?user_email=mark@preludeos.com&days=${days}`);
      // if (response.ok) {
  ...
  const loadDailyReports = async (days) => {
    try {
      // const response = await fetch(`http://localhost:8005/api/analytics/daily-reports?user_email=mark@preludeos.com&days=${days}`);
      // if (response.ok) {
      //   const data = await response.json();
