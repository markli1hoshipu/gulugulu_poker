  ...
  const handleAuthCode = useCallback(async (code, state) => { // <-- state parameter is now expected
    ...
    try {
      ...
      if (tokens.oauth_access_token) {
        ...
        // --- Save OAuth tokens to backend database for auto-refresh ---
        try {
          const CRM_API_URL = import.meta.env.VITE_CRM_API_URL || 'http://localhost:8003';

          const saveResponse = await fetch(`${CRM_API_URL}/api/crm/oauth/save-tokens`, {
            method: 'POST',
            headers: {
          ...
          if (saveResponse.ok) {
            console.log(`✅ Saved ${serviceName} OAuth tokens to backend database`);
          } else {
            console.error(`⚠️ Failed to save ${serviceName} tokens to backend:`, await saveResponse.text());
          }
        } catch (error) {
          console.error(`❌ Error saving ${serviceName} tokens to backend:`, error);
