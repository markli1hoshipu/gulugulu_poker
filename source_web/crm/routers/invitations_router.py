...
def get_db_config() -> dict:
    ...
    if database_url:
        ...
        return {
            ...
            'database': 'prelude_user_management'
        }
    else:
...
async def create_invitation(invitation: InvitationCreate):
    ...
    try:
        ...
        cursor.execute("""
            SELECT id, email FROM user_informations
            WHERE email = %s
        """, (invitation.email,))
        ...
        cursor.execute("""
            INSERT INTO user_profiles (email, company, role, database_name, level)
            VALUES (%s, %s, %s, %s, %s)
...
async def update_invitation(email: str, update_data: InvitationUpdate):
    ...
    try:
        ...
        cursor.execute("""
            SELECT id FROM user_informations
            WHERE email = %s
        """, (email,))
        ...
        if update_data.level is not None:
            ...
            values.append(update_data.level)
        ...
        query = f"""
            ...
            WHERE email = %s
            RETURNING id, email, company, role, database_name, level, created_at, updated_at
        """
