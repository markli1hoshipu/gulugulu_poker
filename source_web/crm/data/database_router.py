...
class DatabaseRouter:
    ...
    def _get_user_management_db_config(self) -> Dict[str, Any]:
        ...
        if database_url:
            ...
            # Use Google Cloud PostgreSQL configuration
            return {
                ...
                'database': 'prelude_user_analytics'  # Changed to use user_analytics DB
            }
    ...
    def get_database_name_for_user(self, email: str) -> str:
        ...
        """
        # Add debug logging for email parameter
        logger.info(f"DEBUG: get_database_name_for_user called with email type: {type(email)}, value: {email}")
        ...
        if isinstance(email, dict):
            logger.error(f"Email parameter is a dict, not a string! Dict: {email}")
            if 'email' in email:
                email = email['email']
                logger.info(f"Extracted email from dict: {email}")
            else:
